...
let state = {
  ...
  zoom: 0,
  parent: '',
  crumbs: [],          // [{zoom, parent, label}] — the drill-down path
  loading: false,
...
function paint(data) {
  ...
  setStatus(`<b>${TIERS[data.zoom]}</b> · ${count} node${count === 1 ? '' : 's'} · ${data.edges.length} link${data.edges.length === 1 ? '' : 's'}`);
}
...
function open() {
  ...
  setRpTab('map');
  state = { ...state, zoom: 0, parent: '', crumbs: [], autoTier: true };
...
export function initTopology() {
  ...
  $('topo-snapshot')?.addEventListener('change', (e) => {
    ...
    state.crumbs = []; state.parent = ''; state.zoom = 0; state.autoTier = true;
    load();
  });
  // Slice C populates this with real incidents; today it documents its own absence
  // rather than pretending the feature is missing.
  $('topo-overlay-pick')?.addEventListener('change', () => { /* no-op until Slice C */ });
