...
| Input | Pin |
| --- | --- |
| Node base image | `node:26-slim@sha256:715e55e4b84e4bb0ff48e49b398a848f08e55daed8eb6a0ea1839ae53bc57583` |
...
| Claude Code CLI | `@anthropic-ai/claude-code@2.1.260` (kept at parity with `@anthropic-ai/claude-agent-sdk`; enforced by `scripts/check-cli-parity.js` in CI) |
...
**Why npm is pinned separately (2026-07-21; base bumped to node 26 2026-07-28).**
`node:26-slim` ships npm 11.17.0, whose bundled node-tar 7.5.16 still carries
**CVE-2026-59873** (CRITICAL, gzip-bomb DoS) — enough to fail the
`image-security` CRITICAL merge gate on every PR. (The prior `node:22-slim` base
had the same problem with npm 10.9.8 / tar 7.5.11.) npm 11.18.0 is the earliest
maintained line bundling the patched tar 7.5.19, and it is installed in the
