 ...
 */
/**
 * A stable key for an external endpoint an event names but the snapshot doesn't
 * contain — the C2 host, the exfil destination. IPs only (the common case): an
 * arbitrary hostname in prose is too noisy to mint a node for. Returns null otherwise.
 */
export function externalActor(token) {
  const t = str(token).toLowerCase();
  if (!t) return null;
  if (/^\d{1,3}(\.\d{1,3}){3}$/.test(t) && t.split('.').every((o) => Number(o) <= 255)) {
    return { key: `ext:${t}`, name: t, ip: t };
  }
  // A compact IPv6 (has a colon and only hex/colon chars) — draw it, don't parse it.
  if (t.includes(':') && /^[0-9a-f:]+$/.test(t) && t.length >= 3) return { key: `ext:${t}`, name: t, ip: t };
  return null;
}

export function buildOverlay(verdict, devices = [], meta = {}) {
  const v = verdict && typeof verdict === 'object' ? verdict : {};
  const resolveDevice = deviceIndex(devices);

  // An event may name an external endpoint (C2, exfil host) that isn't a mapped device.
  // Resolve to a real device when we can; otherwise mint a stable external pseudo-node
  // so the attack still shows where it reached OUTSIDE the estate — that's exactly the
  // part of an incident an analyst wants to see. Collected here, drawn by the client.
  const externals = new Map();
  const resolve = (token) => {
    const dev = resolveDevice(token);
    if (dev) return dev;
    const ext = externalActor(token);
    if (ext) { if (!externals.has(ext.key)) externals.set(ext.key, ext); return ext.key; }
    return '';
  };
  ...
  return {
    ...
    stages,
    entities,
    // External endpoints the incident reached that aren't mapped devices (C2, exfil).
    // The client draws these as standalone nodes at every zoom.
    externals: [...externals.values()].filter((e) => entities.includes(e.key)),
