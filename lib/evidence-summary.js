...
function classifyEvidence(relPath) {
  const normalized = normalizeRelPath(relPath);
  const folder = normalized.split('/')[1];
  if (SUMMARY_SOURCE_DIRS.has(folder)) return folder;
  throw requestError('Summaries are available for records, metrics, detections, and entities evidence JSON.');
}
...
function readJsonFile(session, relPath, { requireSummarizable = false } = {}) {
  ...
  if (requireSummarizable && !canSummarizeEvidencePath(sourcePath)) {
    throw requestError('Summaries are available for JSON evidence under evidence/records, evidence/metrics, evidence/detections, or evidence/entities.');
  }
...
export function summarizeEvidenceValue(relPath, value, context = {}) {
  const sourcePath = normalizeRelPath(relPath);
  const kind = classifyEvidence(sourcePath);
  return {
    sourcePath,
    kind,
    title: sourceTitle(sourcePath),
...
export function jsonValueToCsv(value, relPath = 'data.json') {
  const kind = canSummarizeEvidencePath(relPath) ? classifyEvidence(relPath) : '';
