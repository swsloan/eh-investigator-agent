...
export class ResearchService {
  ...
  async search(input) {
    ...
    if (settings.effectiveProvider === 'brave') {
      try {
        response = await searchBrave(request, this.secretStore?.get?.().braveApiKey, this.braveOptions);
      } catch {
        ...
        response = await searchDuckDuckGo(request, this.duckOptions);
        response.warnings = [
  ...
  async execute(operation, payload = {}) {
    ...
    if (operation === 'fetch') {
      ...
      return fetchReadable(url.href, this.fetchOptions);
    }
    throw new Error('Unknown research operation. Use status, search, or fetch.');
  }
}
