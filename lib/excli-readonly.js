...
// destructive writes. As of excli 0.0.161 one exists: `create_tuningrule`
// (`destructiveHint: true`) can hide detection log entries, so a rule created by
// mistake suppresses future evidence. It is write-class by annotation AND by the
// `create_` prefix, so it cannot execute without the governed approval path.
