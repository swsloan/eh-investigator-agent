...
import { HELP_ARGS, normalizeArg, isMutatingTool } from './excli-readonly.js';
...
export class ExcliBroker {
  ...
  handleRequest(socket, line) {
    ...
    if (this.readOnly && isMutatingTool(argv)) {
    ...
    if (session?.options?.readOnly && isMutatingTool(argv)) {
