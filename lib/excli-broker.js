...
import { recordSafetyEvent, fingerprint } from './safety-log.js';
...
export class ExcliBroker {
  ...
  async executeApproved(action, { workspace } = {}) {
    // Accept 'approved' or 'executing': the decide route flips the record to
    // 'executing' (for live UI feedback + crash visibility) immediately before
    // dispatching here. 'executing' is reachable ONLY from 'approved' per the
    // action-store transition graph, so this preserves the "human-approved"
    // invariant while rejecting proposed/terminal states.
    if (!action || (action.status !== 'approved' && action.status !== 'executing')) {
    ...
    const params = action.params && typeof action.params === 'object' && !Array.isArray(action.params)
      ? action.params
      : {};
    return this._runExcli(capabilityId, params, { cwd });
  }

  /**
   * Run one excli capability in-process and resolve a uniform result
   * `{ ok, exitCode, stdout, stderr, error }`. Shared by the privileged write
   * executor and the read-back verifier so both agree on env, timeout, and error
   * shape. Never rejects — failures are reported in the result. Callers own the
   * read/write gating (executeApproved for writes; observe() for reads).
   */
  _runExcli(capabilityId, params, { cwd }) {
  ...
  /**
   * Run a verifier's read-only probes to observe target state (Phase 3, #23),
   * for both before-execution capture and post-execution read-back. Each probe is
   * `{ capability, params, subject }`. Returns `{ subject: { ok, record, error } }`
   * where `record` is the parsed excli JSON (null if the read failed or returned
   * no JSON). Refuses to run any probe that is not read-class — verification must
   * never itself mutate. Guard/workspace violations reject the promise (uniform
   * error channel with executeApproved); a probe's own excli failure is captured
   * per-subject so a partial read still yields a usable observation set.
   */
  async observe(probes = [], { workspace } = {}) {
    const { cwd } = this.resolveAllowedCwd(workspace);
    const out = {};
    for (const probe of probes) {
      const capability = normalizeArg(probe?.capability);
      const subject = probe?.subject || capability;
      if (!capability) { out[subject] = { ok: false, record: null, error: 'Missing probe capability.' }; continue; }
      if (this.accessTypeForTool(capability) !== 'read') {
        out[subject] = { ok: false, record: null, error: `Refusing to observe via write-class "${capability}".` };
        continue;
      }
      const res = await this._runExcli(capability, probe.params || {}, { cwd });
      let record = null;
      if (res.ok && res.stdout.trim()) {
        try { record = JSON.parse(res.stdout); } catch { record = null; }
      }
      out[subject] = { ok: res.ok, record, error: res.error };
    }
    return out;
  }
  ...
  handleRequest(socket, line) {
    ...
    // A write-class tool is NEVER executable from the agent's socket — not in
    // read-only runs, not in ordinary user sessions, not ever. This used to be
    // conditional on EH_BROKER_READONLY or a per-session flag, neither of which
    // is set in a normal session, so the system prompt's promise that writes are
    // "refused on ./excli-interface" was false exactly where it mattered (#135:
    // a detection was closed on a live appliance by a direct call).
    //
    // This does not affect approved writes: executeApproved() runs them through
    // _runExcli in-process, never through this socket, so the governed
    // propose/approve/verify path is untouched.
    if (isMutatingTool(argv, this.catalog)) {
      this.logger?.warn?.(`[excli-broker] rejected write-class tool "${argv[0]}" (writes require human approval)`);
      // Best-effort session lookup for the audit trail only: the refusal above
      // must not depend on the cwd resolving, or an unrecognised cwd would
      // change the answer from "never" to "it depends".
      let attemptedBy = null;
      try { ({ session: attemptedBy } = this.resolveAllowedCwd(cwd)); } catch { /* refuse anyway */ }
      // Now that a direct write can never succeed, an attempt is a signal rather
      // than routine, so it is always recorded. (No-ops on a null session.)
      recordSafetyEvent(attemptedBy, 'write_refused', {
        tool: argv[0] || '',
        reason: attemptedBy?.options?.readOnly || this.readOnly ? 'read-only run' : 'writes require human approval',
      }); // #32 (observe-only)
      const hint = `./propose-action -json '{"capabilityId":"${argv[0] || 'TOOL'}","params":{...},"label":"<short summary>"}'`;
      sendMessage(socket, {
        error: `"${argv[0]}" is a write action and cannot be executed here — it changes the monitored environment, so it needs human approval. Propose it instead, then report that it is pending; do not claim the change happened:\n  ${hint}`,
      });
      socket.end();
      return;
    }
    ...
    if (cass?.mode === 'replay') {
      ...
      if (hit) {
        if (hit.stdout) { // envelope replayed stdout too, so injection cassettes exercise the boundary
          ...
          const source = `excli ${argv[0] || ''}`.trim();
          const { text, flags } = wrapUntrusted(raw, source);
          if (flags.length) recordSafetyEvent(session, 'injection_suspected', { flags, source, fingerprint: fingerprint(raw) }); // #32
    ...
    child.on('exit', (code, signal) => {
      ...
      const rawStdout = Buffer.concat(out).toString('utf8');
      const source = `excli ${argv[0] || ''}`.trim();
      const { text, flags } = wrapUntrusted(rawStdout, source);
      if (flags.length) recordSafetyEvent(session, 'injection_suspected', { flags, source, fingerprint: fingerprint(rawStdout) }); // #32
      sendMessage(socket, { stream: 'stdout', data: Buffer.from(text, 'utf8').toString('base64') });
