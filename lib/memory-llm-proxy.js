...
function operationPath(req, mountPath) {
  const original = new URL(req.originalUrl || req.url || '/', 'http://memory-proxy.local');
  if (!original.pathname.startsWith(mountPath)) return '';
  return original.pathname.slice(mountPath.length) || '/';
}
...
function readBoundedBody(req, maxBodyBytes) {
...
export function createMemoryLlmProxyHandler({
  ...
  return async function memoryLlmProxy(req, res) {
    ...
    const pathname = operationPath(req, mountPath);
    ...
    try {
      let body;
      try {
        body = await readBoundedBody(req, proxyConfig.maxBodyBytes);
      } catch (error) {
      ...
      timeout = setTimeout(() => controller.abort(new Error('upstream timeout')), proxyConfig.timeoutMs);
      abortUpstream = () => controller.abort(new Error('client aborted request'));
      req.once('aborted', abortUpstream);
      const upstream = await fetchImpl(`https://api.anthropic.com${pathname}`, {
        method,
        headers: {
      ...
      if (abortUpstream) req.off('aborted', abortUpstream);
      activeRequests -= 1;
    }
  };
}
