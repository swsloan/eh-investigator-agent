...
test('a verdict outside the disposition vocabulary is flagged, not silently accepted (#175)', () => {
  const mk = (disposition) => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'cq-vocab-'));
    fs.mkdirSync(path.join(d, 'evidence'));
    fs.writeFileSync(path.join(d, 'evidence', 'verdict.json'), JSON.stringify({
      disposition, confidence: 'high', highest_rung_used: 'records', evidence_chain: [],
    }));
    const flags = (assessConclusionQuality(d).flags || []).filter((f) => f.code === 'invalid_disposition');
    fs.rmSync(d, { recursive: true, force: true });
    return flags;
  };

  // The three spellings actually observed, each on an otherwise-correct run.
  for (const bad of ['true_positive', 'true-positive', 'malicious_true_positive']) {
    assert.equal(mk(bad).length, 1, `"${bad}" must be flagged`);
    assert.equal(mk(bad)[0].severity, 'high');
  }
  assert.equal(mk(undefined).length, 1, 'an absent disposition too');

  // Every legitimate value passes, including an honest refusal to conclude —
  // declining to call it is a valid verdict, not a malformed one.
  for (const ok of ['malicious', 'benign', 'false-positive', 'benign-authorized', 'inconclusive', 'indeterminate']) {
    assert.equal(mk(ok).length, 0, `"${ok}" is legitimate vocabulary`);
  }
});
