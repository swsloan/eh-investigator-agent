...
export const TOPO_LABELS = ['TopoDevice', 'TopoRole', 'TopoSegment', 'TopoLocality', 'TopoIdentity', 'TopoSnapshot', 'TopoEnrichment', 'TopoSegmentName'];
...
export async function writeSnapshot(client, group, { snapshot, nodes, edges, identities, tiers }, layout) {
  ...
  // The write is CREATE-based, so an id that already exists (a re-ingest of the
  // same collected file through POST /topology/ingest — e.g. after a normalization
  // fix) would stack a second copy of every node onto the first. Replace instead.
  await deleteSnapshot(client, group, sid);
...
/* --------------------------------------------------- operator segment names */

/**
 * Name a segment.
 *
 * Telemetry names a segment by what it is on the wire — `vlan:20`, `net:10.42.0.0/24`
 * — because that is all ExtraHop can tell us (Network Localities are Trigger-API
 * only; see DESIGN-network-topology.md). What a segment *means* — "Storage &
 * Backup", "DMZ" — is operator knowledge, so the operator supplies it.
 *
 * Durable and snapshot-independent, exactly like TopoEnrichment: the name is a
 * property of the segment in this environment, not of one observation of it, so it
 * survives re-mapping and pruning. Keyed on the segment key, which is stable
 * across snapshots by construction.
 *
 * An empty name deletes the override, restoring the telemetry name.
 */
export async function writeSegmentName(client, group, key, name) {
  const graph = topologyGraphName(group);
  const segKey = trim(key);
  if (!segKey) return false;
  const label = trim(name).slice(0, 60); // the map draws it; a paragraph is not a name
  const L = assertLabel('TopoSegmentName');
  if (!label) {
    await client.mutate(graph, `MATCH (n:${L} {key: ${cypherStr(segKey)}}) DELETE n`);
    return true;
  }
  await client.mutate(graph,
    `MERGE (n:${L} {key: ${cypherStr(segKey)}}) `
    + `SET n.name = ${cypherStr(label)}, n.named_at = ${cypherStr(new Date().toISOString())}`);
  return true;
}

/** Every operator-assigned segment name in this group, as `{ [key]: name }`. */
export async function readSegmentNames(client, group) {
  const graph = topologyGraphName(group);
  const out = {};
  try {
    const { columns, rows } = await client.query(graph,
      `MATCH (n:${assertLabel('TopoSegmentName')}) RETURN n.key AS key, n.name AS name`);
    const ki = Math.max(0, columns.indexOf('key'));
    const ni = Math.max(0, columns.indexOf('name'));
    for (const row of rows || []) {
      const key = trim(row[ki]);
      const name = trim(row[ni]);
      if (key && name) out[key] = name;
    }
  } catch { /* no graph yet, or no names — an empty map is the right answer */ }
  return out;
}
