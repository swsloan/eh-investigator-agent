...
// Which label + parent field each zoom tier reads from.
const TIER_QUERY = {
  0: { label: 'TopoLocality', parent: null },
  1: { label: 'TopoSegment', parent: 'parent' },
  2: { label: 'TopoRole', parent: 'parent' },
  3: { label: 'TopoDevice', parent: 'role_key' },
};
 ...
 */
export async function readTier(client, group, { snapshotId, zoom = 0, parent = '', limit = 2000 } = {}) {
  const graph = topologyGraphName(group);
  const tier = TIER_QUERY[Math.max(0, Math.min(3, Number(zoom) || 0))];
  const n = Math.max(1, Math.min(5000, Number(limit) || 2000));
  const sid = cypherStr(snapshotId);
  const scope = parent && tier.parent ? ` AND n.${tier.parent} = ${cypherStr(parent)}` : '';
  const extra = tier.label === 'TopoDevice'
    ? `, n.ip AS ip, n.role AS role, n.vlan AS vlan, n.critical AS critical, n.locality AS locality, n.oid AS oid, n.discovery_id AS discovery_id`
  ...
  // Edges only exist between devices; aggregate tiers roll them up by member.
