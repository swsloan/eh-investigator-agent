...
test('the CLI ignores anything a broker sends after the response line', async () => {
  // Regression guard: the response is the first line only. Trailing bytes used
  // to re-enter the parser, which could print the result twice or trip the
  // size cap and turn a delivered answer into a non-zero exit.
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'plan-chatty-'));
  const socketPath = path.join(dir, 'chatty.sock');
  const server = net.createServer((socket) => {
    socket.once('data', () => {
      socket.write(`${JSON.stringify({ result: { ok: true, operation: 'status', revision: 7 } })}\n`);
      socket.write('x'.repeat(900 * 1024)); // past the CLI's 768 KiB response cap
      socket.end();
    });
  });
  ...
  try {
    const { stdout } = await cli(['status'], { workspace: dir, socketPath, capability: 'unused-by-this-stub' });
    const parsed = JSON.parse(stdout);
    assert.equal(parsed.revision, 7, 'the first line was delivered');
    assert.equal(stdout.trimEnd().split('\n').filter((line) => line === '}').length, 1, 'printed exactly once');
  } finally {
