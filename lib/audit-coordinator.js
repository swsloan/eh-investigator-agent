      ...
      switch (event.type) {
        case 'tool_execution_start': {
          ...
          append(session, {
            ...
            tool: toolName,
            summary,
            // #120 slice 0: a delegated call is recorded as its own tool_call, so
            // the trail must say who ran it — otherwise a subagent's work is
            // indistinguishable from the lead's when the trail is reviewed.
            ...(event.parentToolCallId ? { agent: event.agentModel || 'subagent', delegatedBy: event.parentToolCallId } : {}),
          });
          break;
        }
