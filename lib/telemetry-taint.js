...
/**
 * Message for a boundary that the delimiters alone cannot resolve. Two real
 * concatenated envelopes and one envelope whose payload embeds envelope markers
 * produce BYTE-IDENTICAL streams, so no rule can tell them apart and refusing
 * the input would reject every legitimate concatenated file. We take the
 * concatenation reading — the one real telemetry actually produces — and say so
 * out loud, because embedded markers are worth an analyst's attention.
 */
 ...
 * Such lines are DATA and must survive. `wrapUntrusted` always emits its closing
 * tag with nothing but blank lines between it and the next envelope, so a closing
 * tag satisfying that is a candidate terminator and earlier ones are payload.
 *
 * Usually exactly one candidate exists. More than one is genuinely undecidable:
 * a payload closing tag, blank line, and payload opening tag produce a stream
 * byte-identical to two concatenated envelopes. We take the concatenation reading
 * (what real telemetry produces) and report the position via `opts.onAmbiguity`,
 * so `./unwrap` can warn instead of choosing silently. Callers that ignore the
 * hook get the same result as before.
 *
 * Fixing this at the source needs the generator to disambiguate — e.g. a
 * `payload-lines` count on the open tag. Escaping the payload would also work but
 * would mean mutating telemetry, which this module never does (see the header
 * note: annotate, never strip).
 *
...
export function unwrapUntrusted(text, opts = {}) {
  ...
  while (cursor < n && nextOpen[cursor] < n) {
    ...
    push(cursor, open);
    // Each envelope opens with a blank line; drop it so concatenated envelopes
    // do not leave a blank seam between their payloads.
    ...
    const close = candidates.length ? candidates[0] : -1;
