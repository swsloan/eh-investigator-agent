...
// Tools whose output is attacker-controllable telemetry. excli is handled at the
// broker; this covers MCP paths via a PostToolUse hook.
//
//   exmcp  — the ExtraHop MCP surface: records, detections, packets, device data.
//   falcon — CrowdStrike Falcon. Endpoint telemetry is MORE attacker-influenced
//            than wire data, not less: process command lines, script block text,
//            file names and paths, and detection descriptions built from them are
//            all strings an adversary chose. A payload that reaches the model as
//            `falcon_search_detections` output is in exactly the position this
//            envelope exists to mark.
//
// Deliberately NOT matched: mcp__graphiti__ (our own memory store, written by the
// app rather than by an observed adversary). If memory ever ingests raw telemetry
// verbatim, that assumption needs revisiting.
const WIRE_TOOL = /^mcp__(exmcp|falcon)__/;
