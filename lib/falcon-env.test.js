...
test('a write failure is survivable, not fatal', () => {
  ...
  //
  // The unwritable path is constructed rather than hard-coded: a regular file
  // used as a parent directory fails ENOTDIR instantly on every platform. An
  // earlier version used /proc/..., which does not exist on macOS and behaves
  // differently under a container runtime — a portability trap in a test whose
  // whole job is to prove we fail gracefully.
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'falconenv-fail-'));
  const notADir = path.join(dir, 'regular-file');
  fs.writeFileSync(notADir, 'x');
  assert.equal(writeFalconRuntimeEnv({ clientId: 'a' }, path.join(notADir, 'falcon.env')), false);
  fs.rmSync(dir, { recursive: true, force: true });
});
