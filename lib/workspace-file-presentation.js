...
const VERDICT_RE = /^evidence\/verdict\.json$/i;
...
export function classifyWorkspaceFile(session, file) {
  ...
  if (MARKDOWN_RE.test(lower)) {
    ...
    if (isInvestigationPlanArtifact(file.path, content)) {
      presentation = { kind: 'plan', tag: 'PLAN', icon: 'plan', parsed: !empty, sortPriority: -10 };
    } else if (isResearchSummaryArtifact(file.path, content)) {
      presentation = { kind: 'research-summary', tag: 'RESEARCH SUMMARY', icon: 'web-search', parsed: !empty, sortPriority: 0 };
    } else {
      presentation = { kind: 'note', tag: 'NOTE', icon: 'markdown', parsed: !empty, sortPriority: 10 };
    }
  ...
  // The verdict is the deliverable an investigation exists to produce, and it sat
  // outside the approved artifact directories, so it classified as anonymous JSON
  // and was hidden from the workspace listing entirely.
  else if (VERDICT_RE.test(lower)) presentation = { kind: 'verdict', tag: 'VERDICT', icon: 'detection', parsed: !empty, sortPriority: 5 };
  else {
...
export function presentWorkspaceFiles(session, files = session.listFiles()) {
  ...
  // The plan is classified so it can be recognized, then withheld: it has its
  // own ribbon, and listing a generated file beside the agent's deliverables
  // invites hand-editing that the store would overwrite on the next mutation.
  const visiblePresented = presented.filter((file) => file.kind !== 'plan');
  const primaryReport = visiblePresented
    .filter((file) => file.kind === 'report' && file.reveal)
    .sort((a, b) => b.mtime - a.mtime)[0];
  if (primaryReport) primaryReport.primaryReport = true;
  return visiblePresented;
}
