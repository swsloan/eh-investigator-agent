...
test('renderEmbedderEnv emits the three sidecar keys with defaults', () => {
  ...
  assert.match(out, /^OPENAI_API_URL=http:\/\/ollama:11434\/v1$/m);
});
...
test('normalizeEmbedder strips env-file injection from the model name', () => {
  const e = normalizeEmbedder({ model: 'bad\nEVIL=1 name', dimensions: 768, endpoint: 'http://ollama:11434/v1' });
...
test('normalizeEmbedder rejects out-of-range dimensions and bad URLs', () => {
  ...
  assert.equal(normalizeEmbedder({ endpoint: 'file:///etc/passwd' }).endpoint, 'http://ollama:11434/v1');
  assert.equal(normalizeEmbedder({ endpoint: 'not a url' }).endpoint, 'http://ollama:11434/v1');
});
