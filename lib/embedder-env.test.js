...
import { readGraphitiRuntimeGroupId, renderGraphitiRuntimeEnv, writeGraphitiRuntimeEnv } from './embedder-env.js';
...
// ---- A boot that knows nothing must change nothing -------------------------
// The file is bind-mounted from the repo and rewritten at boot, so a local test
// run or a `node server.js` against an empty config used to stamp its derived
// default over the group a configured deployment had already written — silently
// repointing the sidecar on its next restart. That fragmented memory twice.

test('an empty groupId preserves the group already in the file', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'eh-runtime-env-'));
  const file = path.join(dir, 'embedder.env');

  // A configured deployment writes the real group.
  writeGraphitiRuntimeEnv({ embedder: { model: 'nomic-embed-text', dimensions: 768 }, groupId: 'pocextrahop' }, file);
  assert.equal(readGraphitiRuntimeGroupId(file), 'pocextrahop');

  // A boot with no configured group must not overwrite it.
  writeGraphitiRuntimeEnv({ embedder: { model: 'nomic-embed-text', dimensions: 768 } }, file);
  assert.equal(readGraphitiRuntimeGroupId(file), 'pocextrahop', 'the known group survives an ignorant boot');
  assert.match(fs.readFileSync(file, 'utf8'), /GRAPHITI_GROUP_ID=pocextrahop/);
});

test('a known group still wins, so Settings can change it', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'eh-runtime-env-'));
  const file = path.join(dir, 'embedder.env');
  writeGraphitiRuntimeEnv({ groupId: 'pocextrahop' }, file);
  writeGraphitiRuntimeEnv({ groupId: 'otherenv' }, file);
  assert.equal(readGraphitiRuntimeGroupId(file), 'otherenv');
});

test('a fresh install with no file and no group writes no group line', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'eh-runtime-env-'));
  const file = path.join(dir, 'embedder.env');
  writeGraphitiRuntimeEnv({ embedder: { model: 'nomic-embed-text', dimensions: 768 } }, file);
  // config.yaml's own ${GRAPHITI_GROUP_ID:ehdefault} then applies, rather than the
  // app inventing a namespace and persisting it.
  assert.ok(!/GRAPHITI_GROUP_ID/.test(fs.readFileSync(file, 'utf8')));
  assert.equal(readGraphitiRuntimeGroupId(file), '');
});

test('readGraphitiRuntimeGroupId is quiet about a missing file', () => {
  assert.equal(readGraphitiRuntimeGroupId('/nonexistent/embedder.env'), '');
});
