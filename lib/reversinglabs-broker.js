...
export class ReversingLabsBroker {
  ...
  resolveAllowedCwd(cwd) {
    if (!cwd) throw new Error('ReversingLabs broker request did not include a working directory.');
    let realCwd;
    try { realCwd = fs.realpathSync.native(cwd); } catch {
      throw new Error('ReversingLabs broker working directory does not exist.');
    }
    ...
    throw new Error('ReversingLabs broker rejected a request outside a known session workspace.');
  }
}
