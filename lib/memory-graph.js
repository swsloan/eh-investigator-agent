...
/**
 * Escape a string as a single-quoted Cypher literal while PRESERVING line breaks
 * as the `\n` escape sequence. `cypherStr` flattens newlines to spaces (safe for
 * one-line values); summaries are newline-separated accumulations, and flattening
 * them would collapse the per-fact structure and defeat the inspector's
 * summary-line de-duper. A raw newline byte inside a literal breaks FalkorDB's
 * parser, but the two-character `\n` escape stores a real newline correctly, so
 * we emit that. Backslashes are escaped first so the ones we add aren't doubled.
 * Exported for testing.
 */
export function cypherStrKeepNewlines(value) {
...
/**
 * Lean per-group episode + entity counts, for the memory_group preflight check.
 * Deliberately two tiny COUNT queries per group (not the full overview) so it is
 * cheap to run across every namespace at preflight time.
 */
export async function groupCounts(client, groups = []) {
  const out = [];
  for (const group of groups) {
    try {
      const [{ c: episodes = 0 } = {}] = toObjects(await client.query(group, 'MATCH (e:Episodic) RETURN count(e) AS c'));
      const [{ c: entities = 0 } = {}] = toObjects(await client.query(group, 'MATCH (n:Entity) RETURN count(n) AS c'));
      out.push({ group, episodes, entities });
    } catch {
      out.push({ group, episodes: 0, entities: 0, unreadable: true });
    }
  }
  return out;
}
...
/**
 * Pure verdict over group counts — no client, fully unit-testable. `declared` is
 * the operator's configured/env group. Flags the two drift signatures a silent
 * group switch produces: history fragmented across several populated groups, or a
 * declared group that holds nothing while another group has the real history.
 */
export function summarizeGroupHealth(counts = [], { declared = '' } = {}) {
  const populated = counts.filter((c) => c.episodes > 0);
  const byEpisodes = [...populated].sort((a, b) => b.episodes - a.episodes);
  const declaredEntry = declared ? counts.find((c) => c.group === declared) : null;
  const active = declaredEntry?.episodes ? declared : (byEpisodes[0]?.group || declared || null);
  const activeEpisodes = counts.find((c) => c.group === active)?.episodes || 0;
  return {
    active,
    activeEpisodes,
    declared: declared || null,
    fragmented: populated.length > 1,
    declaredEmpty: Boolean(declared) && !declaredEntry?.episodes && populated.length > 0,
    populatedGroups: byEpisodes.map((c) => ({ group: c.group, episodes: c.episodes })),
  };
}
...
/** Max tokens accepted in one resolve call — bounds the generated WHERE clause. */
export const RESOLVE_MAX_TOKENS = 200;

/**
 * Rank a candidate name against the raw token it was matched on. Substring
 * matching alone mis-resolves dotted identifiers — the token `10.0.10.4` is a
 * substring of `10.0.10.40`, a different host — so an exact name wins, then a
 * prefix that ends at a boundary (`10.0.10.4 (DC01)`), then a bare substring.
 * Exported for testing.
 */
export function rankName(name, token) {
  const n = String(name || '').toLowerCase();
  const t = String(token || '').toLowerCase();
  if (!t || !n.includes(t)) return 3;      // no match at all
  if (n === t) return 0;                    // exact
  // A boundary is anything that can't continue the identifier: `10.0.10.4 (DC01)`
  // qualifies, `10.0.10.40` does not.
  if (n.startsWith(t) && !/[a-z0-9.]/.test(n.charAt(t.length))) return 1;
  return 2;                                 // substring
}

/**
 * Resolve many raw tokens (IPs, hostnames, `DOMAIN\user` identities, and
 * `detection:<id>`) against memory in ONE round trip. The investigation panel
 * resolves every entity a run touches; one search per token meant ~31 requests
 * in a burst on a normal investigation.
 *
 * `detection:<id>` is the client's token form for a detection — it is queried on
 * the bare id and prefers a `Detection`-typed node, mirroring what a per-token
 * search did. Resolution policy lives here rather than in the browser so both
 * the query and the ranking stay on one side.
 *
 * Returns `{ [token]: {uuid,name,type,summary} | null }` — null meaning "not in
 * memory", which the caller renders as new-this-run.
 */
export async function resolveTokens(client, graph, tokens = []) {
  const list = [...new Set((Array.isArray(tokens) ? tokens : [])
    .map((t) => String(t ?? '').trim()).filter(Boolean))].slice(0, RESOLVE_MAX_TOKENS);
  const out = {};
  if (!list.length) return out;

  // Each token carries the string actually searched for and an optional type
  // preference; `detection:` is the only prefixed form today.
  const specs = list.map((token) => (token.startsWith('detection:')
    ? { token, q: token.slice('detection:'.length), preferType: 'Detection' }
    : { token, q: token }));
  const queries = [...new Set(specs.map((s) => s.q).filter(Boolean))];
  for (const { token } of specs) out[token] = null;
  if (!queries.length) return out;

  // One pass over the entities, matching any of the tokens. Bounded by the token
  // cap above, and the projection is the same four fields `search` returns.
  const preds = queries.map((q) => `toLower(n.name) CONTAINS ${cypherStr(q.toLowerCase())}`).join(' OR ');
  const rows = toObjects(await client.query(
    graph,
    `MATCH (n:Entity) WHERE ${preds}
     RETURN n.uuid AS uuid, n.name AS name, ${subtypeOf('n')} AS type, n.summary AS summary
     ORDER BY n.name`,
  )).map((r) => ({ ...r, type: r.type || 'Entity' }));

  for (const { token, q, preferType } of specs) {
    let best = null;
    let bestKey = null;
    for (const row of rows) {
      const tier = rankName(row.name, q);
      if (tier === 3) continue;
      // A type preference outranks match quality, the way a per-token search
      // picked the Detection-typed hit before the alphabetically-first one.
      const key = [preferType && row.type !== preferType ? 1 : 0, tier, row.name.length, row.name];
      if (!bestKey || compareKey(key, bestKey) < 0) { best = row; bestKey = key; }
    }
    out[token] = best;
  }
  return out;
...
export async function neighbors(client, graph, uuid) {
  ...
  // Exclude self-referential facts (m = n). Extraction produces them when two
  // mentions of one entity get deduped into a single node ("172.16.204.150 is
  // JJKOUPRO1"), and they carry no neighborhood: kept, they put the focus in its
  // own neighbor list and any renderer draws it twice.
  ...
  for (const r of edgeRows) {
    if (!r.uuid) continue;
    if (r.uuid === focus.uuid) continue; // belt-and-braces for the WHERE above
...
// Relationships that carry a disposition/verdict about the entity — used to tell a
// genuine "the conclusion changed" from a fact merely expiring.
const DISPO_RE = /(disposition|verdict|classif|benign|malicious|false[-\s]?positive|true[-\s]?positive|escalat)/i;
...
export function summarizeInsights(focus, edges, nodesById, episodes) {
  ...
  return {
    ...
    changed_since_prior: dispositionChanged(edges), // the CONCLUSION changed, not just any fact expiring
    highest_risk_rel: best,
  };
}

// True when a prior disposition/verdict about the entity was superseded by a
// *different* one — i.e. the analytic conclusion changed, the signal an analyst
// actually cares about. Stronger than "any fact expired": an expired disposition
// with no differing live replacement (or a non-disposition fact expiring) does
// not count. Falls back to false when the graph carries no disposition edges.
export function dispositionChanged(edges) {
  const dispo = edges.filter((e) => DISPO_RE.test(`${e.rel || ''} ${e.fact || ''}`));
  const expired = dispo.filter((e) => e.expired);
  const live = dispo.filter((e) => !e.expired);
  if (!expired.length || !live.length) return false;
  const key = (e) => `${e.rel || ''}|${e.fact || e.target || ''}`;
  const liveKeys = new Set(live.map(key));
  return expired.some((e) => !liveKeys.has(key(e))); // a prior verdict no longer holds
}
...
export async function mergeNodes(client, graph, canonUuid, dupUuid) {
  ...
  const present = toObjects(await client.query(graph, `MATCH (n:Entity) WHERE n.uuid IN [${C}, ${D}] RETURN n.uuid AS uuid, n.summary AS summary`));
  if (present.length < 2) throw new Error('Both the canonical and duplicate entity must exist.');
  // Compute the folded summary here, not in Cypher. Concatenating in the same
  // query that deletes the duplicate reads d.summary as null (the node is being
  // removed), so `coalesce(c) + '\n' + coalesce(d)` collapses to null and SET
  // then *removes* the canonical's summary — a silent data-loss bug the scratch-
  // namespace test caught. Building the value in JS sidesteps it entirely.
  const summaryOf = (uuid) => present.find((n) => n.uuid === uuid)?.summary || '';
  const foldedSummary = [summaryOf(canonUuid), summaryOf(dupUuid)].filter(Boolean).join('\n');
  ...
  // MERGE, not CREATE: an episode that already mentioned the canonical (common —
  // one run mentions both the IP and its hostname) would otherwise end up with a
  // duplicate MENTIONS edge to it.
  await client.mutate(graph, `MATCH (ep:Episodic)-[m:MENTIONS]->(d:Entity {uuid:${D}})
    MATCH (c:Entity {uuid:${C}})
    MERGE (ep)-[:MENTIONS]->(c)
    DELETE m`);
  // Write the pre-computed summary onto the canonical, then remove the duplicate
  // in a separate query (so d.summary was already read, above).
  await client.mutate(graph, `MATCH (c:Entity {uuid:${C}}) SET c.summary = ${cypherStrKeepNewlines(foldedSummary)}`);
  await client.mutate(graph, `MATCH (d:Entity {uuid:${D}}) DETACH DELETE d`);
