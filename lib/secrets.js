...
// ReversingLabs Spectra Analyze API token. Held by the broker only and scrubbed
// from the agent env, like the ExtraHop secrets.
export const REVERSING_LABS_SECRET_FIELDS = ['reversingLabsApiToken'];
// Brave Search API key (optional web-research provider). Held by the research
// broker only; DuckDuckGo is the zero-account fallback when it is absent.
export const RESEARCH_SECRET_FIELDS = ['braveApiKey'];
// CrowdStrike Falcon API credentials. Held by the app and passed to the
// falcon-mcp sidecar's runtime env file only — never placed in the agent env,
// same posture as the ExtraHop secrets, so a compromised agent process cannot
// read them out of its own environment.
export const FALCON_SECRET_FIELDS = ['falconClientId', 'falconClientSecret'];
...
export const SECRET_FIELDS = [
  ...EXTRA_HOP_SECRET_FIELDS,
  ...REVERSING_LABS_SECRET_FIELDS,
  ...RESEARCH_SECRET_FIELDS,
  ...FALCON_SECRET_FIELDS,
  'anthropicApiKey',
  'claudeOauthToken',
  // Ed25519 private key (PEM) that signs the audit-trail session seal (#30). Held
  // only in the secret store, never exposed to the agent worker; the public key is
  // embedded in each seal so exports verify offline.
  'auditSigningKey',
];
...
// ReversingLabs env keys the agent must never see (config + token). Scrubbed
// from every agent process env; the broker reads the token from the store.
export const REVERSING_LABS_ENV_KEYS = [
  'RL_ENABLED',
  'RL_BASE_URL',
  'RL_API_TOKEN',
  'RL_VERIFY_SSL',
  'RL_ALLOW_CLOUD',
  'REVERSINGLABS_API_TOKEN',
];
// Web-research env keys the agent must never see. Scrubbed from every agent
// process env; the research broker reads the provider/key from config + store.
export const RESEARCH_ENV_KEYS = [
  'EH_RESEARCH_PROVIDER',
  'BRAVE_SEARCH_API_KEY',
];
// Application/infrastructure secrets that live in the app's OWN environment (set
// by docker-compose) but that the agent worker never needs. Scrubbed from every
// agent process env so a worker shell cannot read them (#24):
//   EH_MEMORY_PROXY_TOKEN — the Graphiti sidecar's token for the /memory-llm proxy
//   EH_AUTH_TOKEN         — the UI/API bearer token; a worker holding it could
//                           authenticate to the app as the operator
//   FALKORDB_PASSWORD / FALKORDB_URI — direct access to the memory graph store
//   OPENAI_API_KEY        — the embedder credential (used by the sidecar only)
// The Anthropic model key (ANTHROPIC_API_KEY / OAuth) is intentionally NOT scrubbed
// here — it is the agent's OWN model credential, managed per auth-mode by the
// Claude backend (subscription mode already deletes it). See
// docs/DESIGN-worker-isolation.md for why this env scrub is necessary but not, on
// its own, sufficient (a root worker can still read /proc/1/environ).
export const INFRA_SECRET_ENV_KEYS = [
  'EH_MEMORY_PROXY_TOKEN',
  'EH_AUTH_TOKEN',
  'FALKORDB_PASSWORD',
  'FALKORDB_URI',
  'OPENAI_API_KEY',
...
export function scrubReversingLabsEnv(env = process.env) {
  for (const key of REVERSING_LABS_ENV_KEYS) delete env[key];
}

export function scrubResearchEnv(env = process.env) {
  for (const key of RESEARCH_ENV_KEYS) delete env[key];
}

export function scrubInfraSecretsEnv(env = process.env) {
  for (const key of INFRA_SECRET_ENV_KEYS) delete env[key];
}
...
/** Environment for agent backends: inherited env + additions, app secrets scrubbed. */
export function buildScrubbedEnv(baseEnv = process.env, additions = {}) {
  ...
  scrubInfraSecretsEnv(env);
  // Broker sockets are injected per-session; never inherit a stale one.
  if (!Object.prototype.hasOwnProperty.call(additions, 'EH_REVERSINGLABS_BROKER_SOCKET')) {
    delete env.EH_REVERSINGLABS_BROKER_SOCKET;
  }
  if (!Object.prototype.hasOwnProperty.call(additions, 'EH_RESEARCH_BROKER_SOCKET')) {
    delete env.EH_RESEARCH_BROKER_SOCKET;
  }
  return env;
}
