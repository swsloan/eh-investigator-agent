...
// Values that decline to conclude. Accepted vocabulary — an honest "I could not
// settle this" is a legitimate verdict — so the #175 check must not flag them.
const INCONCLUSIVE = new Set(['inconclusive', 'not-determined', 'undetermined', 'indeterminate', 'undecided']);
...
export function assessConclusionQuality(workspace) {
  ...
  // 0. Vocabulary. A verdict outside the four dispositions is unreadable by
  // every downstream consumer — the scorer, the report templates, any RevealX
  // write-back — and the failure is silent: the run looks finished and the
  // conclusion looks confident. Observed four times across two months, with
  // three spellings ("true_positive", "true-positive",
  // "malicious_true_positive"), each on an investigation that was otherwise
  // right (#175).
  //
  // Flagged rather than corrected. "true positive" is exactly the ambiguity the
  // four-value vocabulary exists to remove: one of those runs used it for a
  // BENIGN true positive (cleartext credentials, a hygiene finding), so mapping
  // the token to `malicious` would invent the opposite verdict.
  const rawDisposition = norm(verdict.disposition);
  if (!DEFINITIVE.has(rawDisposition) && !INCONCLUSIVE.has(rawDisposition)) {
    flags.push({
      severity: 'high',
      code: 'invalid_disposition',
      message: `Verdict disposition "${verdict.disposition ?? '(absent)'}" is not one of `
        + `${[...DEFINITIVE].join(', ')} (or an explicit inconclusive). Downstream consumers `
        + 'cannot read this conclusion; rewrite evidence/verdict.json with a vocabulary value.',
    });
  }
