...
import { summarizeGroupHealth } from './memory-graph.js';
...
function checkCredentials(config, secretStore, credentialsConfiguredFn = credentialsConfigured) {
  ...
  const isRx360 = settings.extrahop.family === 'rx360';
  const hasHost = Boolean(settings.extrahop.host);
  const hasSecret = isRx360
  ...
  const missingMessage = !hasHost
    ? isRx360 ? 'RevealX 360 Tenant ID is not configured.' : 'ExtraHop host is not configured.'
    : !hasSecret
      ? 'ExtraHop credentials are not configured.'
      : 'ExtraHop configuration is incomplete.';
  // Prefer the tenant ID for RevealX 360 so the message matches what the user entered.
  const target = isRx360 ? settings.extrahop.tenantId : settings.extrahop.host;
  return {
    ...
    message: hasCredentials
      ? `ExtraHop credentials are configured${target ? ` for ${isRx360 ? `tenant ${target}` : target}` : ''}.`
      : missingMessage,
  };
}
...
/**
 * Turn a memory-group status into a preflight check. Pure and non-fatal
 * (optional) — a memory-namespace problem should be loudly visible but must not
 * block the app. Split from the async fetch so the verdict logic is unit-tested
 * directly. `status` is null when memory is disabled (check omitted).
 */
export function memoryGroupCheck(status) {
  if (!status) return null;
  const base = { id: 'memory_group', label: 'Memory group', optional: true };
  if (status.unreachable) {
    return { ...base, ok: false, message: 'Memory is enabled but FalkorDB is unreachable; cannot verify the memory namespace.' };
  }
  const health = summarizeGroupHealth(status.counts || [], { declared: status.declared || '' });
  if (health.fragmented) {
    const groups = health.populatedGroups.map((g) => `${g.group} (${g.episodes})`).join(', ');
    return {
      ...base,
      ok: false,
      message: `Investigation memory is split across ${health.populatedGroups.length} groups — ${groups}. New investigations only see the active group, so prior history may be invisible. Align EH_MEMORY_GROUP_ID (and the graphiti sidecar) on one namespace.`,
    };
  }
  if (health.declaredEmpty) {
    return {
      ...base,
      ok: false,
      message: `Configured memory group "${health.declared}" holds no episodes, but "${health.active}" does — writes are likely landing in a different namespace than intended.`,
    };
  }
  if (!health.active || health.activeEpisodes === 0) {
    return { ...base, ok: true, message: 'Memory is enabled; no episodes stored yet.' };
  }
  return { ...base, ok: true, message: `Active memory group "${health.active}" — ${plural(health.activeEpisodes, 'episode')}.` };
}

async function checkMemoryGroup(getMemoryStatus) {
  let status;
  try {
  ...
  return memoryGroupCheck(status);
}
...
export async function getSystemPreflight({
  ...
  if (tuningBroker?.status) checks.push(...tuningBroker.status(fsModule));
  if (investigationPlanBroker?.status) checks.push(...investigationPlanBroker.status(fsModule));
  if (getMemoryStatus) {
    const memoryCheck = await checkMemoryGroup(getMemoryStatus);
