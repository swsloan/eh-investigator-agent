...
import {
  ...
  readNode, readSegmentNames, readTier, writeSegmentName,
...
test('writeSnapshot brackets the ingest with an incomplete → complete flag', async () => {
  ...
  // The replacement deletes come first; the header is the first actual write.
  const first = c.mutations.find((m) => !/DELETE/.test(m.cypher)).cypher;
...
test('writeSnapshot replaces an existing snapshot id instead of stacking a copy onto it', async () => {
  // Observed live: re-ingesting a session (same collected_at → same snapshot id)
  // through POST /topology/ingest doubled every tier — 164 External segments, two
  // contradictory locality rows per name — because the CREATEs landed on top of
  // the previous rows.
  const c = fakeClient();
  await writeSnapshot(c, 'g', SNAP, LAYOUT);
  const firstCreate = c.mutations.findIndex((m) => /CREATE|MERGE/.test(m.cypher));
  const deletes = c.mutations.slice(0, firstCreate).map((m) => m.cypher);
  for (const label of ['TopoDevice', 'TopoRole', 'TopoSegment', 'TopoLocality', 'TopoIdentity']) {
    assert.ok(deletes.some((q) => q.includes(label) && /DETACH DELETE/.test(q)), `${label} cleared before any write`);
  }
  assert.ok(deletes.every((q) => q.includes(`'${SNAP.snapshot.id}'`)), 'scoped to this snapshot id only');
});
...
test('a non-whitelisted label is refused (Cypher label injection boundary)', () => {
  ...
  assert.deepEqual(
    TOPO_LABELS,
    ['TopoDevice', 'TopoRole', 'TopoSegment', 'TopoLocality', 'TopoIdentity', 'TopoSnapshot', 'TopoEnrichment', 'TopoSegmentName'],
  );
});
...
test('an operator segment name is durable and snapshot-independent', async () => {
  const c = fakeClient();
  await writeSegmentName(c, 'g', 'vlan:20', 'Storage & Backup');
  const cypher = c.mutations.at(-1).cypher;
  assert.match(cypher, /MERGE \(n:TopoSegmentName \{key: 'vlan:20'\}\)/);
  assert.match(cypher, /n\.name = 'Storage & Backup'/);
  // No snapshot_id anywhere: the name belongs to the segment, not to one observation
  // of it, so re-mapping and pruning must not take it away.
  assert.ok(!/snapshot_id/.test(cypher), 'never scoped to a snapshot');
  assert.equal(c.mutations.at(-1).graph, 'gtopology', 'sibling graph, not the memory graph');
});

test('an empty name clears the override instead of storing a blank one', async () => {
  const c = fakeClient();
  await writeSegmentName(c, 'g', 'vlan:20', '   ');
  assert.match(c.mutations.at(-1).cypher, /MATCH \(n:TopoSegmentName \{key: 'vlan:20'\}\) DELETE n/);
});

test('a hostile segment name is escaped, and long names are capped', async () => {
  const c = fakeClient();
  await writeSegmentName(c, 'g', 'vlan:20', "x') DETACH DELETE (n) //");
  const cypher = c.mutations.at(-1).cypher;
  assert.ok(!/[^\\]'\) DETACH/.test(cypher), 'the payload cannot break out of the literal');

  await writeSegmentName(c, 'g', 'vlan:21', 'z'.repeat(200));
  const long = c.mutations.at(-1).cypher.match(/n\.name = '(z+)'/);
  assert.equal(long[1].length, 60, 'capped at 60 — the map draws this');
});

test('readSegmentNames returns a key→name map, and survives a missing graph', async () => {
  const c = fakeClient({
    'TopoSegmentName': { columns: ['key', 'name'], rows: [['vlan:20', 'Storage & Backup'], ['vlan:10', 'Core services'], ['vlan:99', '']] },
  });
  const names = await readSegmentNames(c, 'g');
  assert.deepEqual(names, { 'vlan:20': 'Storage & Backup', 'vlan:10': 'Core services' });
  assert.match(c.queries.at(-1).cypher, /MATCH \(n:TopoSegmentName\)/);

  const broken = { query: async () => { throw new Error('no such graph'); } };
  assert.deepEqual(await readSegmentNames(broken, 'g'), {}, 'an unmapped group has no names, not an error');
});

test('deleting a snapshot never deletes the segment names', async () => {
  const c = fakeClient();
  await deleteSnapshot(c, 'g', 'snap-1');
  const all = c.mutations.map((m) => m.cypher).join('\n');
  assert.ok(!/TopoSegmentName/.test(all), 'names outlive the snapshots they were seen in');
  assert.ok(!/TopoEnrichment/.test(all), 'same rule the enrichments already rely on');
});
