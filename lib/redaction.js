const GENERIC_SECRET_PATTERN = /\b(EXTRAHOP_(?:API_KEY|CLIENT_ID|CLIENT_SECRET))\s*[:=]\s*(['"]?)[^\s'",;)}\]]+/gi;
...
export function redactText(text, secretStore) {
  ...
  let out = text.replace(GENERIC_SECRET_PATTERN, (_match, name) => `${name}=[REDACTED]`);
