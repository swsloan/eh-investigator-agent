...
test('sumUsage counts only top-level delegations', () => {
  const u = sumUsage([
    { type: 'tool_execution_start', toolCallId: 't1', toolName: 'Task' },
  ...
  assert.equal(u.delegations, 1, 'a nested delegation is part of its parent, not a new one');
});

test('a delegation counts under either tool name the SDK ships', () => {
  // Observed on a real run: the design was written against `Task`, the SDK in
  // use emits `Agent`. Keying on one name reported zero delegations for a run
  // that made 42 delegated calls — a measurement that silently says "nothing
  // happened" is worse than no measurement.
  const u = sumUsage([
    { type: 'tool_execution_start', toolCallId: 'a1', toolName: 'Agent' },
    { type: 'tool_execution_start', toolCallId: 't1', toolName: 'Task' },
    { type: 'tool_execution_start', toolCallId: 'x1', toolName: 'agent' },
  ]);
  assert.equal(u.delegations, 3, 'Agent, Task and lowercase all count');
});
