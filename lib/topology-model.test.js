...
test('buildHierarchy tags each aggregate with its dominant device role', () => {
  const { tiers } = normalize({
    devices: [
      ...
      { id: '4', ipaddr: '10.0.0.13', role: 'pc', vlanid: '20' },
    ],
  });
  const seg10 = tiers.segments.find((s) => s.key === 'vlan:10');
  const seg20 = tiers.segments.find((s) => s.key === 'vlan:20');
  assert.equal(seg10.role, 'domain_controller', 'the DC segment reads as a DC');
  assert.equal(seg20.role, 'pc', 'the workstation segment reads as pc (the mode)');
  const internal = tiers.localities.find((l) => l.key === 'Internal');
  assert.equal(internal.role, 'pc', 'the locality dominant role is the most common overall');
});
...
test('VLAN 0 means untagged, and never becomes a segment', () => {
  // Two untagged devices on different /24s: without the guard both land in one
  // giant `vlan:0` zone (observed live: 64 of 64 internal devices in one node).
  const { nodes } = normalize({
    devices: [
      { oid: 1, ip: '10.0.1.5', vlan: 0, role: 'server' },
      { oid: 2, ip: '10.0.2.7', vlan: '0', role: 'server' },
      { oid: 3, ip: '10.0.1.6', vlan: 20, role: 'server' },
    ],
  });
  assert.equal(nodes.find((n) => n.oid === '1').segment, 'net:10.0.1.0/24');
  assert.equal(nodes.find((n) => n.oid === '2').segment, 'net:10.0.2.0/24');
  // A real tagged VLAN still wins over the subnet.
  assert.equal(nodes.find((n) => n.oid === '3').segment, 'vlan:20');
  // An untagged device with no usable IP still gets the locality fallback.
  const junk = normalize({ devices: [{ oid: 9, vlan: 0 }] });
  assert.equal(junk.nodes[0].segment, 'loc:Unknown');
});

test('a segment spanning localities is counted into exactly one of them', () => {
  // Same segment key from devices in different localities (a tagged VLAN with a
  // public-IP member). The old rollup pushed the segment into BOTH localities and
  // summed its full device_count into each — Internal 64 / Unknown 64 against an
  // actual 62 / 2 on the live estate.
  const { nodes } = normalize({
    devices: [
      { oid: 1, ip: '10.0.1.5', vlan: 30, role: 'server' },
      { oid: 2, ip: '10.0.1.6', vlan: 30, role: 'server' },
      { oid: 3, ip: '8.8.8.8', vlan: 30, role: 'server' },
    ],
  });
  const t = buildHierarchy(nodes);
  const seg = t.segments.find((s) => s.key === 'vlan:30');
  assert.equal(seg.device_count, 3);
  assert.equal(seg.locality, 'Internal', 'assigned to the majority locality');
  const internal = t.localities.find((l) => l.key === 'Internal');
  const external = t.localities.find((l) => l.key === EXTERNAL_LOCALITY);
  assert.equal(internal.device_count, 3, 'the whole segment counts where it lives');
  assert.equal(external.device_count, 0, 'and never into a second locality');
  assert.ok(!external.members.includes('vlan:30'));
  // Total device weight across localities equals the actual device count.
  const total = t.localities.reduce((s, l) => s + l.device_count, 0);
  assert.equal(total, nodes.length);
});

test('an evenly split segment lands in the same locality whichever order devices arrive', () => {
  // Device enumeration can differ between equivalent snapshots. Without a
  // tiebreak the segment would move localities with nothing about the network
  // having changed — and the drift view would report that as a change.
  const devices = [
    { oid: 1, ip: '10.0.1.5', vlan: 40, role: 'server' },   // Internal
    { oid: 2, ip: '8.8.8.8', vlan: 40, role: 'server' },    // External
  ];
  const forward = buildHierarchy(normalize({ devices }).nodes);
  const reversed = buildHierarchy(normalize({ devices: [...devices].reverse() }).nodes);
  const localityOf = (t) => t.segments.find((s) => s.key === 'vlan:40').locality;
  assert.equal(localityOf(forward), localityOf(reversed), 'a tie resolves the same way both ways round');
  // And the total still equals the real device count, whichever side won.
  for (const t of [forward, reversed]) {
    assert.equal(t.localities.reduce((s, l) => s + l.device_count, 0), 2);
  }
});
