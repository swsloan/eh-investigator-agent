...
const DEFAULTS = {
  ...
  memory: {
    ...
    // group stable across container recreates and gives preflight a value to
    // check the live write group against. NOTE: the graphiti sidecar's actual
    // write group is still set at stack-up from EH_MEMORY_GROUP_ID; the preflight
    // memory_group check surfaces any mismatch. See docs.
    groupId: '',
...
export function deriveGroupId(host, env = process.env, configuredGroup = '') {
  // EH_MEMORY_GROUP_ID wins because it is also what the graphiti sidecar reads
  // (GRAPHITI_GROUP_ID), so the app stays aligned with the actual write group.
  // A persisted config.memory.groupId is the next authority, then the host, then
  // the generic default.
...
export function resolveGroupId(cfg, env = process.env) {
  ...
  let source = 'default';
  if (env.EH_MEMORY_GROUP_ID) source = 'env';
  else if (sanitizeGroupId(cfg?.memory?.groupId)) source = 'config';
