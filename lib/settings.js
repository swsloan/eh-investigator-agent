...
import { EXTRA_HOP_SECRET_FIELDS, scrubExtraHopEnv } from './secrets.js';
...
function normalizeConfig(rawCfg, envConfig = {}) {
...
function envConfigFromAssignments(assignments) {
  ...
  return { extrahop, secrets };
}

export function loadDotEnv(file, { env = process.env } = {}) {
  const fromProcess = {};
  for (const key of Object.keys(env)) {
    if (key.startsWith('EXTRAHOP_')) fromProcess[key] = env[key];
  }
  ...
  if (fs.existsSync(file)) {
    ...
    for (const [key, value] of Object.entries(parsed)) {
      if (key.startsWith('EXTRAHOP_')) assignments[key] = value;
      else env[key] = value;
    }
  }
...
export function loadConfig(file = CONFIG_PATH, { envConfig = {}, secretStore = null } = {}) {
  let rawCfg = {};
  let loaded = false;
  try {
  ...
  const config = normalizeConfig(rawCfg, envConfig);
...
export function buildAgentEnv(cfg, { brokerSocketPath } = {}) {
  const env = {};
