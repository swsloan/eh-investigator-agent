...
const DEFAULTS = {
  ...
  // CrowdStrike Falcon MCP (opt-in). Endpoint telemetry to answer what the wire
  // cannot — process lineage, file writes, removable-media vectors — as an
  // explicit handoff rather than a substitute for ExtraHop evidence.
  falcon: {
    enabled: false,
    url: 'http://falcon-mcp:8000/mcp', // MCP streamable-http endpoint
    // Module allow-list passed to falcon-mcp. The server exposes 100+ tools
    // across 30+ modules; loading all of them would put a large tool surface in
    // every system prompt for a capability used on a minority of investigations
    // (the measured-context-cost concern in #14). Narrow by default.
    modules: 'detections,hosts,incidents,intel',
    // CrowdStrike API host for your cloud (api.crowdstrike.com, api.us-2, api.eu-1).
    baseUrl: 'https://api.crowdstrike.com',
  },
  memory: {
    enabled: false, // Graphiti temporal-memory MCP server (opt-in)
    url: 'http://graphiti-mcp:8000/mcp', // MCP HTTP endpoint
    // Persisted memory namespace. Empty = derive from the ExtraHop host (see
    // deriveGroupId). Persisting it here (config_data volume) keeps the declared
    // group stable across container recreates. This is the authority: the app
    // writes it into the graphiti runtime env file, so the sidecar follows it on
    // its next start. EH_MEMORY_GROUP_ID is only the fallback when unset.
    groupId: '',
    // Embedder used by the Graphiti sidecar. Any OpenAI-compatible embedding
    // endpoint works (the local llama.cpp `embeddings` service by default). Read
    // by the sidecar at startup via graphiti/runtime/embedder.env, so changes
    // apply on a memory-stack restart.
    embedder: {
      model: 'nomic-embed-text', // must match the endpoint's model name
      dimensions: 768, // vector size; must match the model (nomic=768, OpenAI 3-*=1536)
      endpoint: 'http://embeddings:8080/v1', // OpenAI-compatible base URL
    },
  },
  extrahop: {
...
// Embedder model / endpoint names are written verbatim into an env file the
// Graphiti sidecar reads, so sanitize hard: strip anything outside a safe set
// (no newlines/quotes/spaces → no env-file injection), and bound the length.
function sanitizeEmbedderToken(value, fallback, extraChars = '') {
  if (typeof value !== 'string') return fallback;
  const cleaned = value.trim().replace(new RegExp(`[^A-Za-z0-9._:${extraChars}-]`, 'g'), '');
  return cleaned ? cleaned.slice(0, 200) : fallback;
}

function sanitizeEmbedderEndpoint(value, fallback) {
  if (typeof value !== 'string' || !value.trim()) return fallback;
  const trimmed = value.trim();
  try {
    const url = new URL(trimmed);
    if (url.protocol !== 'http:' && url.protocol !== 'https:') return fallback;
    // A URL can't contain whitespace/newlines once parsed; re-serialize to be safe.
...
function normalizeEmbedderDimensions(value, fallback) {
  const n = typeof value === 'number' ? value : parseInt(String(value ?? '').trim(), 10);
  if (!Number.isInteger(n) || n < 1 || n > 16384) return fallback;
  return n;
}

export function normalizeEmbedder(raw = {}) {
  const def = DEFAULTS.memory.embedder;
  return {
    // Model names may contain '/' (e.g. 'library/model') and '@' (tags), so allow them.
    model: sanitizeEmbedderToken(raw.model, def.model, '/@'),
    dimensions: normalizeEmbedderDimensions(raw.dimensions, def.dimensions),
    endpoint: sanitizeEmbedderEndpoint(raw.endpoint, def.endpoint),
  };
}

// FalkorDB RediSearch requires an alphanumeric group_id. Lowercase, strip
// everything else, and bound the length. Empty result means "not configured" —
// deriveGroupId then falls back to the host-derived namespace.
export function sanitizeGroupId(value) {
  return String(value ?? '').toLowerCase().replace(/[^a-z0-9]+/g, '').slice(0, 63);
}

/** Falcon MCP settings. `enabled` alone does nothing without credentials in the
 *  secret store — server.js only advertises the server when both are present. */
function normalizeFalcon(raw = {}) {
  const modules = typeof raw.modules === 'string' ? raw.modules.trim() : '';
  return {
    enabled: Boolean(raw.enabled),
    url: (typeof raw.url === 'string' && raw.url.trim()) || DEFAULTS.falcon.url,
    // Comma-separated module names; anything else falls back to the default set
    // rather than being passed through to the server's CLI.
    modules: /^[a-z0-9_,-]*$/i.test(modules) && modules ? modules : DEFAULTS.falcon.modules,
    // Rendered into the sidecar env file, so it must not carry whitespace or
    // quotes that could break out of KEY=value.
    baseUrl: /^https?:\/\/[A-Za-z0-9.:_~+-]+\/?$/.test(String(raw.baseUrl || '').trim())
      ? String(raw.baseUrl).trim()
      : DEFAULTS.falcon.baseUrl,
  };
}
...
function normalizeMemory(raw = {}) {
  return {
    ...
    groupId: sanitizeGroupId(raw.groupId),
    embedder: normalizeEmbedder(raw.embedder),
  };
}
...
// RevealX 360 addressing. The user supplies only a Tenant ID; the fixed API
// host is constructed from it. Legacy web/API hosts are recognized and reduced
// back to the tenant ID for compatibility with older stored configs.
const RX360_API_SUFFIX = '.api.cloud.extrahop.com';
const RX360_WEB_SUFFIX = '.cloud.extrahop.com';
const RX360_TENANT_ID_RE = /^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/;

/** Recover a tenant ID from current IDs and legacy RevealX 360 web/API hosts. */
export function rx360TenantIdFromTarget(value) {
  const raw = typeof value === 'string' ? value.trim().toLowerCase() : '';
  if (!raw) return '';
  if (RX360_TENANT_ID_RE.test(raw)) return raw;

  let hostname = raw.replace(/\.$/, '');
  if (raw.includes('://')) {
    try {
      const url = new URL(raw);
      if (url.username || url.password || url.port || (url.pathname && url.pathname !== '/') || url.search || url.hash) return '';
  ...
  for (const suffix of [RX360_API_SUFFIX, RX360_WEB_SUFFIX]) {
    if (!hostname.endsWith(suffix)) continue;
    const tenantId = hostname.slice(0, -suffix.length);
    return RX360_TENANT_ID_RE.test(tenantId) ? tenantId : '';
  }
  return '';
}

/** Construct the fixed RevealX 360 API hostname from a user-entered tenant ID. */
export function rx360ApiHostFromTenantId(value) {
  const tenantId = typeof value === 'string' ? value.trim().toLowerCase() : '';
  if (!tenantId) return '';
  if (!RX360_TENANT_ID_RE.test(tenantId)) {
    const error = new Error('Tenant ID must contain only the tenant name (for example, "tenant"), not a URL or hostname.');
    error.code = 'INVALID_RX360_TENANT_ID';
    throw error;
  }
  return `${tenantId}${RX360_API_SUFFIX}`;
}
...
function normalizeConfig(rawCfg, envConfig = {}, integrationConfig = {}) {
  ...
  return {
    ...
    falcon: normalizeFalcon(cfg.falcon),
 ...
 */
export function deriveGroupId(host, env = process.env, configuredGroup = '') {
  // The UI setting wins. The app is the sole writer of the sidecar's
  // GRAPHITI_GROUP_ID, so nothing outside this resolution can contradict it, and
  // a setting that a stale .env line could silently shadow is exactly the
  // hidden-config trap that fragmented memory in the first place. This also
  // matches how the embedder settings superseded their .env knobs.
  // EH_MEMORY_GROUP_ID remains the deployment default for when nothing is
  // configured (fresh installs, CI), then the host, then the generic default.
  if (configuredGroup) return sanitizeGroupId(configuredGroup);
...
/**
 * The effective memory namespace plus its origin, so the UI can show what is
 * actually in force and say so when an environment override is shadowing the
 * Settings → Memory choice. Value comes from deriveGroupId, so there is exactly
 * one precedence implementation.
 */
export function resolveGroupId(cfg, env = process.env) {
  const value = deriveGroupId(cfg?.extrahop?.host, env, cfg?.memory?.groupId);
  let source = 'default';
  if (sanitizeGroupId(cfg?.memory?.groupId)) source = 'config';
  else if (env.EH_MEMORY_GROUP_ID) source = 'env';
  else if (String(cfg?.extrahop?.host || '').replace(/[^a-z0-9]+/gi, '')) source = 'host';
  return { value, source };
}
...
export function buildAgentEnv(cfg, {
  ...
  tuningBrokerSocketPath,
  actionBrokerSocketPath,
  investigationPlanBrokerSocketPath,
  ...
  // Suppression visibility: read-only tuning-rule access over REST. Always
  // exposed when the socket exists — it cannot write, and knowing what
  // `search_detections` is hiding matters on every investigation. Inert without
  // ExtraHop credentials, which the broker resolves (never the agent).
  if (tuningBrokerSocketPath) env.EH_TUNING_BROKER_SOCKET = tuningBrokerSocketPath;
  // Structured investigation plans. Both halves are required: the socket alone
  // is inert, because the broker authorizes every request against a per-session
  // capability and refuses one whose workspace does not match the caller's cwd.
  if (investigationPlanBrokerSocketPath && investigationPlanCapability) {
    env.EH_INVESTIGATION_PLAN_BROKER_SOCKET = investigationPlanBrokerSocketPath;
    env.EH_INVESTIGATION_PLAN_CAPABILITY = investigationPlanCapability;
  }
  ...
  if (cfg?.memory?.enabled && cfg?.memory?.url) {
    ...
    env.EH_MEMORY_GROUP_ID = deriveGroupId(cfg?.extrahop?.host, process.env, cfg?.memory?.groupId);
  }
  return env;
}
...
export function buildExcliEnv(cfg, secretStore, baseEnv = process.env) {
  ...
  if (eh.family === 'rx360') {
    // Canonicalize whatever is stored (tenant ID or legacy host) to the API host.
    const tenantId = rx360TenantIdFromTarget(eh.host);
    if (tenantId) env.EXTRAHOP_HOST = rx360ApiHostFromTenantId(tenantId);
  ...
  } else if (eh.host && !isPlainHost(eh.host)) {
    // A config saved before this validation existed (or edited by hand) could
    // still hold a URL-shaped host. excli honours userinfo and would dial the
    // host after the "@" (#134), so withhold BOTH the host and the credential:
    // excli then reports "not configured", which is accurate and safe. The
    // alternative — passing it through — mails the API key to that host.
    // eslint-disable-next-line no-console
    console.error(`[settings] refusing to pass an invalid ExtraHop host to excli: ${JSON.stringify(eh.host)} — fix it in Settings → ExtraHop.`);
  } else {
...
export function credentialsConfigured(cfg, secretStore) {
  ...
  const secrets = secretStore?.get?.() || {};
  if (eh.family === 'rx360') {
    return Boolean(rx360TenantIdFromTarget(eh.host) && secrets.clientId && secrets.clientSecret);
  }
  return Boolean(eh.host && isPlainHost(eh.host) && secrets.apiKey);
}
...
export function publicSettings(cfg, secretStore) {
  ...
  const falcon = resolved.falcon || {};
  return {
    backend: resolved.backend,
    falcon: {
      enabled: Boolean(falcon.enabled),
      url: falcon.url || '',
      baseUrl: falcon.baseUrl || '',
      modules: falcon.modules || '',
      // Whether the credential exists, never the credential.
      credentialsSet: Boolean(secrets.falconClientId && secrets.falconClientSecret),
      // What the agent actually sees: the MCP server is advertised only when the
      // toggle AND the credentials are both present (server.js falconMcpServers).
      configured: Boolean(falcon.enabled && secrets.falconClientId && secrets.falconClientSecret),
    },
    integrations: {
      reversingLabs: {
    ...
    memory: {
      ...normalizeMemory(cfg.memory),
      // Read-only companions for the Settings → Memory group picker.
      ...(() => { const r = resolveGroupId(cfg); return { groupIdEffective: r.value, groupIdSource: r.source }; })(),
    },
    ...
    extrahop: {
      ...
      host: eh.host || '',
      // RevealX 360 exposes the tenant ID (derived from the stored host) so the
      // UI can present only the Tenant ID field; enterprise leaves it empty.
      tenantId: eh.family === 'rx360' ? rx360TenantIdFromTarget(eh.host) : '',
...
export function applyUpdate(cfg, body, { secretStore = null } = {}) {
  ...
  if (body.falcon && typeof body.falcon === 'object') {
    if (typeof body.falcon.enabled === 'boolean') next.falcon.enabled = body.falcon.enabled;
    if (typeof body.falcon.url === 'string' && body.falcon.url.trim()) next.falcon.url = body.falcon.url.trim();
    // Re-normalized rather than assigned: the module list becomes argv for the
    // falcon-mcp sidecar, so it must not pass through unchecked.
    if (typeof body.falcon.modules === 'string') {
      next.falcon = normalizeFalcon({ ...next.falcon, modules: body.falcon.modules });
    }
    if (typeof body.falcon.baseUrl === 'string') {
      next.falcon = normalizeFalcon({ ...next.falcon, baseUrl: body.falcon.baseUrl });
    }
  }
  if (body.memory && typeof body.memory === 'object') {
    ...
    if (typeof body.memory.groupId === 'string') next.memory.groupId = sanitizeGroupId(body.memory.groupId);
    if (body.memory.embedder && typeof body.memory.embedder === 'object') {
      // Merge onto the current embedder so a partial patch keeps unset fields.
      next.memory.embedder = normalizeEmbedder({ ...next.memory.embedder, ...body.memory.embedder });
    }
  }
  const eh = body.extrahop || {};
  if (eh.family === 'enterprise' || eh.family === 'rx360') next.extrahop.family = eh.family;
  if (next.extrahop.family === 'rx360') {
    if (typeof eh.tenantId === 'string') {
      next.extrahop.host = rx360ApiHostFromTenantId(eh.tenantId);
    } else if (typeof eh.host === 'string') {
      // Compatibility for older clients that still submit the shared Host
      // field. Recognized web/API hosts are canonicalized to the API host;
      // an unrecognized non-empty host throws INVALID_RX360_TENANT_ID.
      const tenantId = rx360TenantIdFromTarget(eh.host);
      if (eh.host.trim() && !tenantId) rx360ApiHostFromTenantId(eh.host);
      next.extrahop.host = tenantId ? rx360ApiHostFromTenantId(tenantId) : '';
    }
  } else if (typeof eh.host === 'string') {
    // Validate on the way in, so a URL pasted into the Host field is refused
    // with a clear message rather than silently becoming a request target.
    next.extrahop.host = eh.host.trim() ? assertPlainHost(eh.host) : '';
  }
  ...
  // CrowdStrike Falcon API client. Same convention: '' = keep, '-' = clear.
  // Accepted under `falcon` so the credential and its toggle arrive in one patch.
  for (const [bodyKey, secretKey] of [['clientId', 'falconClientId'], ['clientSecret', 'falconClientSecret']]) {
    const v = body.falcon?.[bodyKey];
    if (typeof v === 'string' && v !== '') secretPatch[secretKey] = v === '-' ? '' : v.trim();
  }
