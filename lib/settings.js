...
import { EXTRA_HOP_SECRET_FIELDS, REVERSING_LABS_ENV_KEYS, scrubExtraHopEnv, scrubReversingLabsEnv } from './secrets.js';
...
const DEFAULTS = {
  ...
  // Third-party enrichment integrations. The API token lives in the secret
  // store (never here); only non-secret settings are persisted.
  integrations: {
    reversingLabs: {
...
function envConfigFromAssignments(assignments) {
  const extrahop = {};
  const integrations = { reversingLabs: {} };
  const secrets = {};
...
const INTEGRATION_ENV_KEYS = new Set(REVERSING_LABS_ENV_KEYS);
...
export function applyUpdate(cfg, body, { secretStore = null } = {}) {
  ...
  if (!next.integrations) next.integrations = { reversingLabs: normalizeReversingLabs() };
