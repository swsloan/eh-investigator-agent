...
import { claudeNativeBinaryStatus } from '../../claude-native.js';
...
async function detect(execFile = execFileCallback) {
  // Sessions run through the SDK's bundled, arch-native CLI binary — not the
  // global `claude` on PATH — so if that binary is missing for this machine's
  // architecture, the backend cannot work no matter what `claude --version`
  // says. Report it as unavailable with the concrete fix (rebuild for this arch)
  // rather than letting it surface as a failed agent turn.
  const native = claudeNativeBinaryStatus();
  if (!native.ok) {
