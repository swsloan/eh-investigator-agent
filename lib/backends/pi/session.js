...
export class PiSession extends AgentSession {
  ...
  ensureProcess() {
    ...
    // control plane is root, lower the pi CLI (and its Bash children) to it. Pi
    // already spawns directly, so this is just uid/gid + HOME. workerSpawnUser may
    // throw (fail closed) when the hardened profile is set but we aren't root.
    const spawnUser = workerSpawnUser();
    const env = buildScrubbedEnv(process.env, this.options.env || {});
    if (spawnUser) env.HOME = spawnUser.home;
    const proc = spawn('pi', args, {
      cwd: this.workspace,
      env,
      stdio: ['pipe', 'pipe', 'pipe'],
      ...(spawnUser ? { uid: spawnUser.uid, gid: spawnUser.gid } : {}),
    });
    this.proc = proc;
