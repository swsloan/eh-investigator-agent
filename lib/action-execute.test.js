...
test('executeApproved runs only on an approved/executing action', async () => {
  if (!ExcliBroker) return;
  const broker = brokerWithCatalog();
  // Rejected: not-yet-approved and terminal states.
  ...
  await assert.rejects(() => broker.executeApproved(approved({ status: 'rejected' }), { workspace: '/tmp' }), /approved action/i);
  // 'executing' (the state the decide route dispatches in) passes the status gate
  // and reaches the workspace guard — i.e. it is NOT rejected for its status.
  await assert.rejects(() => broker.executeApproved(approved({ status: 'executing' }), { workspace: '/tmp' }), /workspace|outside|does not exist/i);
});
