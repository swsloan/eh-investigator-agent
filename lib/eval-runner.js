...
import { loadPrevDetails } from '../eval/harness/history.js';
...
// #128: eval disposal deletes the workspace, so the artifacts that explain a
// behavioural result have to be copied out before it runs.
import { retainCaseArtifacts } from './eval-artifacts.js';
// Usage accounting moved to its own module when the external live harness needed
// it too (GET /:id/usage). Re-exported here so existing importers are unchanged.
import { sumUsage, usageAsCaseMeta } from './session-usage.js';
...
export async function runEvalInApp({
  ...
  runId, timestamp, gateTarget = 0.05, costCeiling = null, accuracyFloor = 0.8, falseAlarmTarget, accuracyDropLimit,
  caseIds = null, maxParallel = 3,
    ...
    try {
      ...
      if (verdict) {
        ...
        results[c.id] = {
          ...
          // cost/tokens plus the #120 delegation breakdown, under the field
          // names the scorer reads. Shared with the live harness so the two
          // paths cannot report the same measurement under different keys.
          ...usageAsCaseMeta(usage),
      ...
      // Retain before disposal, and in `finally` rather than the success path: a
      // case that threw part-way is exactly the one whose trace is worth having.
      retainCaseArtifacts({
        workspace: session.workspace,
        transcript: session.transcript || [],
        outDir: path.join(reportsDir, 'artifacts', runId, c.id),
      });
  ...
  const { record, detail } = scoreRun({
    ...
    // undefined keeps the scorer's default (#144); an explicit value overrides it.
    ...(falseAlarmTarget === undefined ? {} : { falseAlarmTarget }),
    ...(accuracyDropLimit === undefined ? {} : { accuracyDropLimit }),
    meta: { run_id: runId, timestamp, backend: backendId, ...meta },
    // #127: a window of prior runs, not one sample — a regression flag has to
    // be able to tell a step change from a case that flips.
    prevDetails: loadPrevDetails(reportsDir, backendId),
  });
