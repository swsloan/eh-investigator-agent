...
export class ActionIndex {
  ...
  /**
   * Live view for the dashboard: open actions whose session still exists, with
   * titles resolved via `getTitle(sessionId)` (return null/undefined to drop an
   * action whose session was deleted — such entries are also evicted here, so the
   * index self-heals without a delete hook), oldest-first, plus `pendingCount`
   * (actions awaiting a human decision = 'proposed').
   */
  snapshot(getTitle = () => '') {
    const actions = [];
    for (const a of [...this.open.values()]) {
      const sessionTitle = getTitle(a.sessionId);
      if (sessionTitle == null) { this.open.delete(a.id); continue; } // session gone — evict
      actions.push({ ...a, sessionTitle });
    }
