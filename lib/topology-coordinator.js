...
import { deleteSnapshot, latestSnapshotId, listSnapshots, readTier, writeEnrichments, writeSnapshot } from './topology-store.js';
...
export const ENRICHMENT_ARTIFACT = path.join('evidence', 'topology', 'enrichments.json');
...
export function createTopologyCoordinator({ client, getConfig, resolveGroup, logger = console } = {}) {
  ...
  /**
   * Merge the on-demand device enrichments the agent wrote (Slice 5). Separate from
   * the topology ingest: enrichments are keyed by device and stored WITHOUT a
   * snapshot_id, so they persist across snapshots and are safe to merge on any turn.
   * Best-effort; never throws.
   */
  async function mergeEnrichments(workspace, { group } = {}) {
    if (!enabled() || !workspace) return { ok: false, merged: 0 };
    let entries;
    try {
      const raw = fs.readFileSync(path.join(workspace, ENRICHMENT_ARTIFACT), 'utf8');
      const parsed = JSON.parse(raw);
      entries = Array.isArray(parsed) ? parsed : (Array.isArray(parsed?.enrichments) ? parsed.enrichments : null);
    } catch {
      return { ok: false, merged: 0 };
    }
    if (!entries?.length) return { ok: false, merged: 0 };
    const namespace = group || resolveGroup?.() || '';
    const merged = await writeEnrichments(client, namespace, entries);
    return { ok: merged > 0, merged, group: namespace };
  }

  async function onAgentEnd(session, meta = {}) {
    if (!enabled()) return;
    if (meta.hadError || meta.promptSource !== 'user') return;
    // Enrichments can arrive on any turn (the map's enrichment prompt), independent of
    // whether a fresh topology.json was produced — merge them first, unconditionally.
    try {
      const enr = await mergeEnrichments(session.workspace);
      if (enr.merged) logger.info?.(`[topology:${session.id?.slice(0, 8)}] merged ${enr.merged} device enrichment(s)`);
    } catch (err) {
      logger.warn?.(`[topology:${session.id?.slice(0, 8)}] enrichment merge failed: ${err?.message || err}`);
    }
  ...
  return { attachSession, ingest, mergeEnrichments, enabled };
}
