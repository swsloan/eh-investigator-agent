...
/**
 * One tuning rule by id. The id goes into the path as the caller gave it: routing
 * a decimal string through Number() rounds anything past 2^53 (so
 * "9007199254740993" would fetch ...992 — a different rule, or a 404), and an id
 * is an opaque identifier rather than a quantity.
 */
export async function getTuningRule(id, deps = {}) {
  let idPath;
  if (typeof id === 'number') {
    if (!Number.isSafeInteger(id) || id < 0) {
      throw new Error('A numeric tuning rule id must be a non-negative safe integer; pass larger ids as a string.');
    }
    idPath = String(id);
  ...
  return extrahopGet(`${HIDING_RULES_PATH}/${idPath}`, deps);
}
