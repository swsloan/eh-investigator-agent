...
import { summarizeInsights, dispositionChanged, summarizeGroupHealth, groupCounts, neighbors, cypherStrKeepNewlines, mergeNodes, resolveTokens, rankName, RESOLVE_MAX_TOKENS } from './memory-graph.js';
...
// ---------- merge: summary fold + escaping ----------

test('cypherStrKeepNewlines preserves line breaks as the \\n escape, not raw bytes', () => {
  assert.equal(cypherStrKeepNewlines('a\nb'), "'a\\nb'", 'real newline -> two-char escape');
  assert.equal(cypherStrKeepNewlines('a\r\nb'), "'a\\nb'", 'CRLF collapses to one escape');
  assert.equal(cypherStrKeepNewlines("o'brien"), "'o\\'brien'", 'single quote escaped');
  assert.equal(cypherStrKeepNewlines('c:\\x'), "'c:\\\\x'", 'backslash escaped before quotes/newlines');
  assert.equal(cypherStrKeepNewlines(null), "''");
});

test('mergeNodes folds both summaries and never reads d.summary in the delete query', async () => {
  const mutations = [];
  const client = {
    async query(graph, cypher) {
      // The presence check must return BOTH summaries so the fold happens in JS.
      return {
        columns: ['uuid', 'summary'],
        rows: [['canon', 'Canon line.'], ['dup', 'Dup line.']],
      };
    },
    async mutate(graph, cypher) { mutations.push(cypher); return { columns: [], rows: [] }; },
  };
  const out = await mergeNodes(client, 'g', 'canon', 'dup');
  assert.deepEqual(out, { canon: 'canon', dup: 'dup', merged: true });

  const setSummary = mutations.find((m) => /SET c\.summary =/.test(m));
  assert.ok(setSummary, 'the summary is written');
  assert.match(setSummary, /'Canon line\.\\nDup line\.'/, 'both lines, joined by the \\n escape');
  assert.doesNotMatch(setSummary, /DELETE/, 'the summary SET does not also delete — that null-reads d.summary');
  assert.doesNotMatch(setSummary, /coalesce/, 'no in-Cypher concat of the deleted node');

  // MENTIONS is rewired with MERGE (dedupe), not CREATE (which duplicates).
  const mentions = mutations.find((m) => /MENTIONS/.test(m) && /DELETE m/.test(m));
  assert.match(mentions, /MERGE \(ep\)-\[:MENTIONS\]->\(c\)/);
});

test('mergeNodes tolerates a duplicate with no summary', async () => {
  const client = {
    async query() { return { columns: ['uuid', 'summary'], rows: [['canon', 'Only canon.'], ['dup', null]] }; },
    async mutate() { return { columns: [], rows: [] }; },
  };
  // present.length is 2, so it proceeds; the fold just keeps the canon line.
  const captured = [];
  client.mutate = async (g, c) => { captured.push(c); return { columns: [], rows: [] }; };
  await mergeNodes(client, 'g', 'canon', 'dup');
  const set = captured.find((m) => /SET c\.summary/.test(m));
  assert.match(set, /'Only canon\.'/);
  assert.doesNotMatch(set, /\\n/, 'no trailing separator when the dup had no summary');
});

// ---------- batch token resolution ----------

// Stub graph: one query, whatever the WHERE clause, returns these entities.
const resolveClient = (rows, seen = []) => ({
  async query(graph, cypher) {
    seen.push(cypher);
    return {
      columns: ['uuid', 'name', 'type', 'summary'],
      rows: rows.map((r) => [r.uuid, r.name, r.type, r.summary || '']),
    };
  },
});

test('rankName prefers exact, then a boundary prefix, then a bare substring', () => {
  assert.equal(rankName('10.0.10.4', '10.0.10.4'), 0);
  assert.equal(rankName('10.0.10.4 (DC01)', '10.0.10.4'), 1, 'prefix ending at a space');
  assert.equal(rankName('proxy-10.0.10.4', '10.0.10.4'), 2, 'substring, not a prefix');
  assert.equal(rankName('10.0.10.40', '10.0.10.4'), 2, 'a longer IP is NOT a boundary prefix');
  assert.equal(rankName('WEB01', '10.0.10.4'), 3, 'no match');
  assert.equal(rankName('DC01', 'dc01'), 0, 'case-insensitive');
});

test('resolveTokens answers every token from a single query', async () => {
  const seen = [];
  const client = resolveClient([
    { uuid: 'a', name: '10.0.10.4 (DC01)', type: 'Device' },
    { uuid: 'b', name: 'proxy.acmelegal.lab', type: 'Device' },
  ], seen);
  const out = await resolveTokens(client, 'g', ['10.0.10.4', 'proxy.acmelegal.lab', '203.0.113.9']);
  assert.equal(seen.length, 1, 'one round trip for the whole batch');
  assert.equal(out['10.0.10.4'].uuid, 'a');
  assert.equal(out['proxy.acmelegal.lab'].uuid, 'b');
  assert.equal(out['203.0.113.9'], null, 'unmatched tokens resolve to null, not missing keys');
});

test('resolveTokens does not let a longer IP swallow a shorter one', async () => {
  // The substring-only rule this replaced resolved .4 to whichever name sorted
  // first — including the unrelated host .40.
  const client = resolveClient([
    { uuid: 'long', name: '10.0.10.40', type: 'Device' },
    { uuid: 'real', name: '10.0.10.4 (DC01)', type: 'Device' },
  ]);
  const out = await resolveTokens(client, 'g', ['10.0.10.4']);
  assert.equal(out['10.0.10.4'].uuid, 'real');
});

test('resolveTokens queries a detection id and prefers the Detection node', async () => {
  const seen = [];
  const client = resolveClient([
    { uuid: 'note', name: 'note about 4294968382', type: 'Investigation' },
    { uuid: 'det', name: 'detection_id 4294968382', type: 'Detection' },
  ], seen);
  const out = await resolveTokens(client, 'g', ['detection:4294968382']);
  assert.match(seen[0], /4294968382/);
  assert.doesNotMatch(seen[0], /detection:/, 'the prefix is stripped before querying');
  assert.equal(out['detection:4294968382'].uuid, 'det', 'type preference outranks the shorter name');
});

test('resolveTokens dedupes, ignores blanks, and caps the batch', async () => {
  const seen = [];
  const client = resolveClient([], seen);
  const out = await resolveTokens(client, 'g', ['x', 'x', '  ', null, 'y']);
  assert.deepEqual(Object.keys(out).sort(), ['x', 'y']);
  const many = Array.from({ length: RESOLVE_MAX_TOKENS + 50 }, (_, i) => `t${i}`);
  const capped = await resolveTokens(resolveClient([]), 'g', many);
  assert.equal(Object.keys(capped).length, RESOLVE_MAX_TOKENS);
});

test('resolveTokens makes no query when there is nothing to resolve', async () => {
  const seen = [];
  assert.deepEqual(await resolveTokens(resolveClient([], seen), 'g', []), {});
  assert.deepEqual(await resolveTokens(resolveClient([], seen), 'g', ['   ']), {});
  assert.equal(seen.length, 0);
});

test('resolveTokens escapes quotes in tokens rather than breaking the query', async () => {
  const seen = [];
  await resolveTokens(resolveClient([], seen), 'g', ["o'brien\\lab"]);
  assert.doesNotMatch(seen[0], /(^|[^\\])'o'brien/, 'the embedded quote is escaped');
  assert.match(seen[0], /CONTAINS '/);
});
