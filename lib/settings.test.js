...
test('resolveGroupId reports the effective namespace and where it came from', () => {
  ...
  // An env override must be reported as such so the UI can say the setting is inert.
  assert.deepEqual(resolveGroupId(cfg, { EH_MEMORY_GROUP_ID: 'pocextrahop' }), { value: 'pocextrahop', source: 'env' });
  assert.deepEqual(resolveGroupId({ memory: { groupId: 'pocextrahop' } }, {}), { value: 'pocextrahop', source: 'config' });
