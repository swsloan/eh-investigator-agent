...
// Classify a research-fetch failure and, when it is an SSRF/exfil guard block,
// record it to the session's safety log (#32) — observe-only, reason only (the
// blocked destination/error carries no secret). Other failures aren't safety
// events. The fetch guard messages come from lib/research/fetch.js.
function recordResearchGuardBlock(session, err) {
  if (!session) return;
  const msg = String(err?.message || '');
  if (/rejected a (local or internal hostname|non-public destination|hostname that resolves)/i.test(msg)) {
    recordSafetyEvent(session, 'ssrf_blocked', { reason: 'local/internal/non-public destination' });
  } else if (/exceeded the .* response limit/i.test(msg)) {
    recordSafetyEvent(session, 'exfil_blocked', { reason: 'response exceeded the size limit' });
  }
}
...
export class ResearchBroker {
  ...
  async handleRequest(socket, line, context) {
    ...
    try {
      ...
      ({ session } = resolveBrokerWorkspace(this.sessions, typeof request.cwd === 'string' ? request.cwd : '', { brokerName: 'Research broker' }));
      ...
      recordResearchGuardBlock(session, err); // #32 safety log (observe-only)
