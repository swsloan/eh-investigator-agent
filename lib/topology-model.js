...
function normalizeNode(raw, rules) {
  ...
  // VLAN 0 is 802.1Q for "untagged" — it means no VLAN, not a VLAN named 0. Left
  // as a segment key it swallows the whole untagged estate into one zone (observed:
  // 64 of 64 internal devices in `vlan:0`), so it falls through to the subnet.
  const vlanRaw = str(raw.vlan ?? raw.vlanid);
  const vlan = vlanRaw === '0' ? '' : vlanRaw;
...
export function buildHierarchy(nodes = []) {
  ...
  // A segment spanning localities (devices sharing a segment key but not a
  // locality) must still live in exactly ONE place on the map: pushing it into
  // every member's locality counted its full device_count into each of them.
  // Assign it to the locality most of its devices are in, and derive locality
  // membership from that single assignment.
  const segLocality = new Map();
  for (const n of nodes) {
    let m = segLocality.get(n.segment);
    if (!m) { m = new Map(); segLocality.set(n.segment, m); }
    m.set(n.locality, (m.get(n.locality) || 0) + 1);
  }
  // Ties break on the locality name, not on device enumeration order. Two
  // equivalent snapshots can list the same devices in a different order, and
  // without the tiebreak an evenly-split segment would move between localities
  // with nothing about the network having changed — which the drift view would
  // then report as a change.
  const majority = (m) => {
    let best = ''; let bestN = 0;
    for (const [k, c] of (m || [])) {
      if (c > bestN || (c === bestN && best && k < best)) { best = k; bestN = c; }
    }
    return best;
  };
  for (const s of segments.values()) {
    s.locality = majority(segLocality.get(s.key)) || s.locality;
    localities.get(s.locality)?.members.push(s.key);
  }
  ...
  // Dominant device role per segment (and locality), so the map can colour an
  // aggregate by the kind of devices it mostly holds — a server subnet reads
  // differently from a workstation one. A role tier's dominant role is its own role.
  const tally = (nodesList, keyOf) => {
    const counts = new Map();
    for (const n of nodesList) {
      const k = keyOf(n);
      let c = counts.get(k);
      if (!c) { c = new Map(); counts.set(k, c); }
      c.set(n.role, (c.get(n.role) || 0) + 1);
    }
    return counts;
  };
  const mode = (m) => { let best = ''; let bestN = 0; for (const [role, c] of (m || [])) if (c > bestN) { best = role; bestN = c; } return best; };
  const segRoles = tally(nodes, (n) => n.segment);
  const locRoles = tally(nodes, (n) => n.locality);
  for (const r of out.roles) r.role = r.name;
  for (const s of out.segments) s.role = mode(segRoles.get(s.key));
  for (const l of out.localities) l.role = mode(locRoles.get(l.key));
  return out;
}
