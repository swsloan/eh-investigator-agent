...
import { verifySignature } from './audit-keys.js';
 ...
 */
export function verifyTrail(text, { trustedKeyIds = null } = {}) {
  const lines = String(text || '').split('\n').map((l) => l.trim()).filter(Boolean);
  let prev = GENESIS;
  let expectedSeq = 1;
  let lastSeal = null;      // the most recent seal entry
  let unsealedAfterSeal = 0; // entries appended after that seal (not yet re-sealed)
  for (let i = 0; i < lines.length; i++) {
    ...
    if (payload.type === 'seal') { lastSeal = { ...payload, prevHash, index: i }; unsealedAfterSeal = 0; }
    else if (lastSeal) unsealedAfterSeal += 1;
    prev = hash;
    expectedSeq += 1;
  }

  const result = { ok: true, entries: lines.length, head: prev, sealed: false };
  if (lastSeal) {
    // Authenticity: the seal signs the chain head as of the seal (== its prevHash).
    let sealOk = lastSeal.root === lastSeal.prevHash
      && verifySignature(lastSeal.root, lastSeal.sig || '', lastSeal.pubKey || '');
    let sealError = sealOk ? null : 'seal signature does not verify against its embedded public key';
    if (sealOk && trustedKeyIds && !trustedKeyIds.includes(lastSeal.keyId)) {
      sealOk = false;
      sealError = `seal key ${lastSeal.keyId} is not in the trusted key set`;
    }
    result.sealed = true;
    result.seal = { keyId: lastSeal.keyId || null, ok: sealOk, root: lastSeal.root, error: sealError };
    result.unsealedAfterSeal = unsealedAfterSeal;
  }
  return result;
}
...
export class AuditTrail {
  ...
  /**
   * Seal the trail (issue #30 layer 3): sign the current chain head with the
   * signer's Ed25519 key and append a chained `seal` entry embedding the
   * signature, keyId, and public key (so an export verifies offline). Returns the
   * seal line, or null when the trail is empty / signing fails. `signer` is
   * `{ keyId, publicKeyPem, sign(data)->b64 }` from lib/audit-keys.js.
   */
  seal(workspace, signer) {
    const head = this.headFor(workspace);
    if (head.hash === GENESIS) return null; // nothing to seal
    try {
      const sig = signer.sign(head.hash);
      return this.append(workspace, {
        type: 'seal', root: head.hash, sig, keyId: signer.keyId, pubKey: signer.publicKeyPem, alg: 'ed25519',
      });
    } catch (err) {
      this.logger?.warn?.(`[audit-trail] seal failed: ${err?.message || err}`);
      return null;
    }
  }
